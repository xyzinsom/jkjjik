...
END:VCARD`.trim()
    ...
    */
    ...
    conn.reply = async (jid, text = '', quoted, options) => {
        let lah = global.shiro
        const _uptime = process.uptime() * 1000
        const u = conn.clockString(_uptime)
        return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) :
  await conn.generateMessage(jid, { text, contextInfo: { externalAdReply: { title: 'Hitori Bocchi', body: 'By Asrori Amin', mediaType: 1, previewType: 0, showAdAttribution: true, renderLargerThumbnail: true, thumbnail: await getBuffer(lah), thumbnailUrl: lah, sourceUrl: 'https://chat.whatsapp.com/FovMd3tev2EKIfrqcFEviT' } }}, { quoted })
    }
