let handler = async(m, { conn, usedPrefix, command, text, args }) => {
    let args1 = args[0]
    let args2 = args[1]
    if (isNaN(args2)) return m.reply(`contoh:\n${usedPrefix + command} @${m.sender.split`@`[0]} 7`)
    let who = m.mentionedJid ? m.mentionedJid[0] : (args2.replace(/[@ .+-]/g, '').replace(' ', '') + '@s.whatsapp.net')
...
STATUS SUCCESS`
    ...
    } else { throw `Limit anda tidak mencukupi untuk mentransfer limit sebanyak ${args2}`
    }
